...
class Gaussian_Stationary:
    ...
    def train(self):
        ...
        self.df_train_res['y_pred'] = y_pred.flatten()
        self.df_train_res['y_std'] = y_std.flatten()
        ...
        plt.plot(self.df_train_res["Healthcare"], color='blue')
        plt.savefig("../Output/pred_delta_train.png")
        plt.close()

        # Generate predictions for test
        y_pred, y_std = self.gp1.predict(self.x_test_res_1, return_std=True)
        self.df_test_res['y_pred'] = y_pred.flatten()
        self.df_test_res['y_std'] = y_std.flatten()
        ...
        plt.figure(figsize=(20, 5))
        plt.plot(self.df_test_res["y_pred"], color='blue')
        plt.plot(self.df_test_res["Healthcare"], color='red')
        ...
        if not os.path.exists("../Output"):
            os.makedirs("../Output")
        pickle.dump(self.gp1, open("../Output/model.pkl", "wb"))
    ...
    def split_data(self, data_df_res, df_comp):
        ...
        X = pd.to_datetime(df_comp["month"]).map(pd.Timestamp.toordinal)
        ...
        self.df_train_res = data_df_res[:-test_size].copy()
        self.df_test_res = data_df_res[-test_size:].copy()
        ...
        self.x_train_res_1 = x_train_res.values.reshape(-1, 1)
        self.y_train_res_1 = y_train_res.values.reshape(-1, 1)
        self.x_test_res_1 = x_test_res.values.reshape(-1, 1)
        self.y_test_res_1 = y_test_res.values.reshape(-1, 1)


# Main execution
if __name__ == "__main__":
    data = pd.read_csv("../Input/CallCenterData.csv")
    df_comp = data.copy()
    data_df_res = data.copy()
    model = Gaussian_Stationary(df_comp, data_df_res)
    print("Training complete. Model and plots saved in Output folder.")
